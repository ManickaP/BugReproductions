...
public class Handle {
    ...
    //public Handle(IAsyncDisposable? disposable) {
    public Handle(IDisposable? disposable) {
        _disposable = disposable;
    }

    public void Release() {
        Console.WriteLine($"Release");
        //var _ = _disposable?.DisposeAsync();
        _disposable?.Dispose();
    }
...
public unsafe struct Struct : IDisposable {
    public byte* _buffers;
    public int _count;
    public void Allocate(int count)
    {
        Console.WriteLine($"Allocate {count}");
        _buffers = (byte*)NativeMemory.AllocZeroed((nuint)count);
        _count = count;
        Console.WriteLine($"Allocated {(nuint)_buffers}");
    }
    public void Dispose()
    {
        Console.WriteLine($"Dispose {(nuint)_buffers}");
...
public class X {
    ...
    public Struct _struct = new Struct();

    public void A()
    {
        _struct.Allocate(5);
    }
    public unsafe void Print(string str)
    {
        Console.WriteLine($"{str}: {(nuint)_struct._buffers} {_struct._count}");
    }
}
...
partial class Program
{
    public static void Test() {
        ...
        x.Print("1");
        x.A();
        ref Struct s = ref x._struct;
        var handle = new Handle(s);
        x.Print("2");
        handle.Release();
        x.Print("3");
    }
